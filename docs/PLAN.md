...
Users and workspaces. A `User` (email, password hash, display name, role `user | admin`, preferred language) owns one implicit personal workspace: every `KnowledgeModel`, `FIP`, `Session` and user-contributed `FER` carries an `ownerId`. System content (the GO FAIR model, the seed FER catalogue) has no owner and is read-only for everyone except admins. Each owned object has a `visibility`: `private` (owner only), `link` (anyone with the URL can read), `public` (listed). FIPs created anonymously inside a workshop session have `ownerId = null`, belong to the session, and carry an `editToken` (kept in the participant's browser) so only that device can edit them; a signed-in participant can *claim* such a FIP into their workspace at any time. Since 9 Sep a FIP can also be started anonymously outside any session (*standalone*: `ownerId = null`, `sessionId = null`, edit token, visibility `link` by default); it is listed nowhere and lives through its edit link until claimed (docs/specs/09-standalone-fips.md).
...
- Understanding and assurance layer (docs/specs/12-understanding-assurance-dashboard.md, proposal 10 Sep): entailments per question, type-conformance prompt, coherence rules, evidence per declaration and an assurance level in the matrix, assessments read from the nanopublication network, automated probes.
- FIP dashboard across a session, this instance's public FIPs and network FIPs: principle coverage, resource adoption, similarity and convergence, gaps, evolution (spec 12 §C).
...
- **Authorization:** every read and write is checked against `ownerId`, `visibility`, session membership (join code) or edit token. Anonymous access exists through a session join link, a `link`/`public` object URL, or a standalone FIP started from the home page (rate-limited per IP, switchable off with `FIPM_ANONYMOUS_FIPS`).
...
9. Decided 9 Sep with the user: FIPs can be filled independently of any workshop session and without an account (standalone FIPs, docs/specs/09-standalone-fips.md). Defaults taken without facilitators: visibility `link`, 30 creations per hour per IP, feature on by default (`FIPM_ANONYMOUS_FIPS`), the privacy notice's session paragraph extended to cover them, retention as for session data (12 months after last edit, enforced by the `purge-standalone-fips` CLI command run by the operator, no scheduler shipped). The editor offers an edit link (`/fips/{id}/edit?token=…`, token stored by the browser and stripped from the address bar) so a FIP can move between devices; sharing it grants edit rights, which the share box says.
10. Co-facilitator feedback, 10 Sep ("seria legal ter todas as opções de respostas que eu mandei e, em Outros, abrir uma caixa para o pesquisador escrever o Outro"): every option from the area document is shown. Options that name a catalogue resource stay `suggestedFerIds`; descriptive phrases ("apenas texto não estruturado", "conta do repositório"…) become `suggestedPhrases` that tick into free-text declarations and never enter the FER catalogue; a built-in "Outro (especificar)" box adds the researcher's own wording (docs/specs/10-suggested-phrases-and-other.md). Order: catalogue options, phrases, Other.
11. Decided 10 Sep with the user: start the nanopublication-network integration now rather than after the workshop. Scope: read existing FIPs from the network (Nanopub Query) and show them; "prepare for export" generates our FIPs as unsigned nanopublications ready for a later signing step. No publishing, no keys, no network writes in this iteration (docs/specs/11-nanopub-network.md).
12. Lead's criticism, 10 Sep: FIPs are self-assertions and, more fundamentally, presuppose an understanding of the principles that most respondents lack (65 of 385 options in the co-facilitator's document are type-mismatched). Recorded as spec 12 with a proposed pre-CONFOA slice (entailments per question, type-conformance prompt, evidence links with a declared/evidenced badge) to decide by the 12 Sep scope freeze; the rest is v2.
13. Decided 11 Sep with the user: build the FIP dashboard (spec 12 §C) now, designed for growth — "the numbers may increase significantly so the application should be prepared to deal with this". Design in docs/specs/13-fip-dashboard.md: a write-maintained declaration projection instead of aggregating the answers JSON, precomputed per-FIP similarity signatures with candidate generation instead of all-pairs scoring, and a snapshot layer, targeting 10k–100k FIPs on SQLite today and Postgres later. Similarity weighting defaults to equal weight per principle (assumption, to confirm; spec 12 §E.3).
